...
def data_loader(data, label, batch_size, seq_len):
    ...
    while i < len(data):
        ...
        i += 1
    train_x_tensor = torch.tensor(np.array(x, dtype=np.float32))
    train_y_tensor = torch.tensor(np.array(y, dtype=np.float32).reshape(-1,1))
...
def kfold(X, ret, label, num_fold, is_quantile):
    ...
    for i in range(num_fold):
        ...
        if train1_end_idx is not None:
            train_x = pd.concat([train_x, X[:train1_end_idx]],axis=1)
            train_y = pd.concat([train_y, y[:train1_end_idx]])
        if train2_start_idx is not None:
            train_x = pd.concat([train_x, X[train2_start_idx:]], axis=1)
            train_y = pd.concat([train_y, y[train2_start_idx:]], axis=1)
        val_x = X[val_start_idx:val_end_idx]
        val_y = y[val_start_idx:val_end_idx]

        print(f'{p.ordinal(i+1)} fold learning')
        taccu, vaccu = train_test(train_x, train_y, val_x, val_y, is_quantile)
        taccus.append(taccu)
        vaccus.append(vaccu)
        print()
        val_end_idx -= val_len
        ...
        val_start_idx = val_end_idx - val_len
        val_start_date = X.index[val_start_idx]
        train2_start_date = val_start_date + pd.DateOffset(months=1) + pd.DateOffset(weeks=2)
        train2_start_idx = X.index[train2_start_date]
        if i == num_fold:
            train1_end_idx = None
            train1_end_date = None
        else:
            train1_end_date = val_start_date - pd.DateOffset(months=1)
            train1_end_idx = X.index[train1_end_date]
    print(f'Final Result  taccu {round(sum(taccus) / num_fold, 4)}  vaccu {round(sum(vaccus) / num_fold, 4)}')
    return

def train_test(train_x, train_y, val_x, val_y, is_quantile):
    tlosses = []
    vlosses = []
    if is_quantile:
    ...
    for epoch in range(epochs):
        ...
        vaccu /= total_len
        print(f'Epoch {epoch+1}  accuracy {round(taccu, 4)}  loss {round(avg_loss, 4)}  '\
        f'val accuracy {round(vaccu, 4)}  val loss {round(val_avg_loss, 4)}')
